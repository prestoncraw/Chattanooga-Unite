...
export default async function getAuthUser(user) {
    ...
    let query = `SELECT u.*, sp.name, sp.id as sp_id FROM users u JOIN service_providers sp ON sp.owner_id = u.id WHERE u.email = ?`;
    ...
    if (userResults.length === 2) {
        ...
        const userCheck = await executeQuery({ query, values }, debug);
        let is_admin = JSON.parse(userCheck)[0]["is_admin"];

        if (debug) console.log(`User is_admin: ${is_admin}`);
        authUser.is_admin = is_admin;

        let uID = JSON.parse(userCheck)[0]["id"];
        if (debug) console.log(`User id: ${uID}`)
        authUser.id = uID;
    }
    else {
        ...
        if (debug) console.log(`User id: ${userResultsObj[0].id}`)
        authUser.id = userResultsObj[0].id;

        // add organizations to authUser object
        for (let i = 0; i < userResultsObj.length; i++) {
            authUser.Organizations.push({name: userResultsObj[i].name, id: userResultsObj[i].sp_id});
        }
    }
