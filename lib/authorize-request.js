...
// this function takes in 4 parameters: the request & response, and the "auth level" and (optional) target org
...
export async function authorizeRequest(req, res, authLevel, targetOrg) {
...
export async function getAuthUserID(req, res) {
    const session = await getServerSession(req, res);
    const user = await getAuthUser(session.user);
    return user.id;
}